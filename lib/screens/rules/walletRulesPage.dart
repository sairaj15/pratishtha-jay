...
class WalletRulesPage extends StatefulWidget {
  @override
  _WalletRulesPageState createState() => _WalletRulesPageState();
}

class _WalletRulesPageState extends State<WalletRulesPage> {
  @override
  Widget build(BuildContext context) {
    return FutureBuilder(
        ...
        builder: (context, AsyncSnapshot<Map> snapshot){
          if(snapshot.hasData){
            if(!snapshot.data!.containsKey(ruleKeys[8])){
              return Center(
                child: noContentWidget(),
              );
            }
            return RulesWidget(rules: snapshot.data![ruleKeys[8]]["rules"]);
